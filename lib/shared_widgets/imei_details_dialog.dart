...
void showImeiDetailsDialog(BuildContext context, String imeiNo) {
  ...
  showDialog(
    ...
    builder: (dialogContext) {
      // 🟢 اردو الائنمنٹ کے لیے RTL سپورٹ
      return Directionality(
        textDirection: TextDirection.rtl,
        child: stockItem == null
            ? _buildNotFoundDialog(dialogContext, imeiNo)
            : _buildDetailsDialog(dialogContext, stockItem),
      );
    },
  );
}

/// ❌ جب IMEI کا ڈیٹا نہ ملے
Widget _buildNotFoundDialog(BuildContext dialogContext, String imeiNo) {
  return Dialog(
    ...
    child: Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'اس IMEI ($imeiNo) کا ڈیٹا اسٹاک میں نہیں ملا۔',
...
/// 🟢 جب IMEI کا ڈیٹا مل جائے
Widget _buildDetailsDialog(BuildContext dialogContext, dynamic stockItem) {
  final Map<String, dynamic> data =
      Map<String, dynamic>.from(stockItem as Map);
    ...
    child: Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                ...
                child:
                    const Icon(Icons.phone_android, color: Colors.red, size: 20),
              ),
          ...
          _buildInfoRow(
            'RAM / ROM:',
            '${data['ram'] ?? 'N/A'} / ${data['rom'] ?? 'N/A'}',
          ),
          _buildInfoRow(
            'حالت (Condition):',
            data['condition'] == 'new' ? 'نیا (New)' : 'پرانا (Old)',
          ),
